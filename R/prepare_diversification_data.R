...
# Put BAMM in a hidden folder (not included in the package build)
...
# Ask path for folder used to store input/output files generated
# Ask if the folder should be kept or erased
...
# BAMM_install_folder_path <- "./software/bamm-2.5.0/"

# Check if BAMM works with non ultrametric trees (fossils)

# library(phytools)
# data(eel.tree)
#
# BAMM_data <- prepare_diversification_data(BAMM_install_folder_path = "./software/bamm-2.5.0/",
#    phylo = eel.tree,
#    prefix_for_files = "eel",
#    numberOfGenerations = 50000,
#    additional_BAMM_parameters = list(updateLambdaShiftScale = 0.5),
#    plot_evaluations = TRUE)


prepare_diversification_data <- function (BAMM_install_folder_path, # Ask the path to folder where is the BAMM 'executable'.
                                          phylo, # Phylogeny. Object of class phylo. Must be rooted and fully resolved.
                                          prefix_for_files = NULL, # To provide the prefix to add to all BAMM files stored in the 'BAMM_output_folder_path' and kept if 'keep_BAMM_outputs = TRUE'.
                                          ...
                                          sampleProbsFilename = NULL, # Provide path to clade-specific sampling fractions. See ?BAMMtools::samplingProbs() to generate the file. If provided, 'globalSamplingFraction' is ignored.
                                          expectedNumberOfShifts = NULL, # Set the expected number of regime shifts used to set the exponential hyperprior used to modulate reversible jumps across model configuration in the rjMCMC run.
                                                                         # If set to NULL (default), will use an empirical rule to define this value such as we expected a regime shift for every 100 tips in the phylogeny.
                                          ...
                                          additional_BAMM_parameters = list(), # Additional optional parameters. See details in the template file provided with the package files './BAMM_template_diversification.txt'
                                          BAMM_output_folder_path = "./BAMM_outputs/", # Ask path for folder used to store input/output files generated
                                          keep_BAMM_outputs = TRUE, # Ask if the folder should be kept or erased
...

{
  ...
  {
    # BAMM_install_folder_path must be a folder, so no '.' in the path (except at the beginning if using relative path)

    # additional_BAMM_parameters => Check if they match the list of available parameters

    # Incompatibility across parameters
      # When requesting burn-in and nb_posterior_samples that are not compatible with the number of remaining samples after burn-in
        # Check combination of numberOfGenerations, eventDataWriteFreq, burn_in, and nb_posterior_samples
  ...
  ## Get names of the additional parameters
  add_pars_names <- names(additional_BAMM_parameters)

  ## Create output folder if missing
  if (!dir.exists(paths = file.path(BAMM_output_folder_path)))
  {
    dir.create(path = file.path(BAMM_output_folder_path))
  }
  ...
  {
    phy_path <- file.path(paste0(BAMM_output_folder_path, "phylogeny.tree"))
  } else {
    phy_path <- file.path(paste0(BAMM_output_folder_path, prefix_for_files,"_phylogeny.tree"))
  }
  # Export tree
  ...
  # If failed, load it from the /inst/ folder as in source/bundled version of the package
  ...
  if ("runInfoFilename" %in% add_pars_names)
  {
    runInfoFilename <- file.path(additional_BAMM_parameters$runInfoFilename)
  } else {
  ...
  if ("sampleFromPriorOnly" %in% add_pars_names)
  {
    sampleFromPriorOnly <- additional_BAMM_parameters$sampleFromPriorOnly
  } else {
  ...
  if ("runMCMC" %in% add_pars_names)
  {
    runMCMC <- additional_BAMM_parameters$runMCMC
  } else {
  ...
  if ("simulatePriorShifts" %in% add_pars_names)
  {
    simulatePriorShifts <- additional_BAMM_parameters$simulatePriorShifts
  } else {
  ...
  if ("loadEventData" %in% add_pars_names)
  {
    loadEventData <- additional_BAMM_parameters$loadEventData
  } else {
  ...
  if ("eventDataInfile" %in% add_pars_names)
  {
    eventDataInfile <- file.path(additional_BAMM_parameters$eventDataInfile)
  } else {
  ...
  if ("initializeModel" %in% add_pars_names)
  {
    initializeModel <- additional_BAMM_parameters$initializeModel
  } else {
  ...
  if ("overwrite" %in% add_pars_names)
  {
    overwrite <- additional_BAMM_parameters$overwrite
  } else {
  ...
  if ("validateEventConfiguration" %in% add_pars_names)
  {
    validateEventConfiguration <- additional_BAMM_parameters$validateEventConfiguration
  } else {
  ...
  if (is.null(prefix_for_files))
  {
    priors_path <- file.path(paste0(BAMM_output_folder_path, "priors.txt"))
  } else {
    priors_path <- file.path(paste0(BAMM_output_folder_path, prefix_for_files,"_priors.txt"))
  }
  ...
  {
    expectedNumberOfShifts <- round(length(phylo$tip.label)/100) # 1 expected regime shift for 100 tips
  }
  ...
  if ("lambdaInitPrior" %in% add_pars_names)
  {
    # If provided as 'additional_BAMM_parameters', use this value
    lambdaInitPrior <- additional_BAMM_parameters$lambdaInitPrior
  } else {
  ...
  if ("lambdaShiftPrior" %in% add_pars_names)
  {
    # If provided as 'additional_BAMM_parameters', use this value
    lambdaShiftPrior <- additional_BAMM_parameters$lambdaShiftPrior
  } else {
    # If not provided, use the default value suggested by BAMMtools::setBAMMpriors
  ...
  if ("muInitPrior" %in% add_pars_names)
  {
    # If provided as 'additional_BAMM_parameters', use this value
    muInitPrior <- additional_BAMM_parameters$muInitPrior
  } else {
    # If not provided, use the default value suggested by BAMMtools::setBAMMpriors
  ...
  if ("lambdaIsTimeVariablePrior" %in% add_pars_names)
  {
    lambdaIsTimeVariablePrior <- additional_BAMM_parameters$lambdaIsTimeVariablePrior
  } else {
  ...
  if ("mcmcOutfile" %in% add_pars_names)
  {
    mcmcOutfile <- file.path(additional_BAMM_parameters$mcmcOutfile)
  } else {
  ...
  if ("mcmcWriteFreq" %in% add_pars_names)
  {
    mcmcWriteFreq <- additional_BAMM_parameters$mcmcWriteFreq
  } else {
  ...
  if ("eventDataOutfile" %in% add_pars_names)
  {
    eventDataOutfile <- file.path(additional_BAMM_parameters$eventDataOutfile)
  } else {
  ...
  if ("printFreq" %in% add_pars_names)
  {
    printFreq <- additional_BAMM_parameters$printFreq
  } else {
  ...
  if ("outputAcceptanceInfo" %in% add_pars_names)
  {
    outputAcceptanceInfo <- additional_BAMM_parameters$outputAcceptanceInfo
  } else {
  ...
  if ("acceptanceInfoFileName" %in% add_pars_names)
  {
    acceptanceInfoFileName <- file.path(additional_BAMM_parameters$acceptanceInfoFileName)
  } else {
  ...
  if ("acceptanceResetFreq" %in% add_pars_names)
  {
    acceptanceResetFreq <- additional_BAMM_parameters$acceptanceResetFreq
  } else {
    if (numberOfGenerations >= 10^6)
    {
  ...
  if ("updateLambdaInitScale" %in% add_pars_names)
  {
    updateLambdaInitScale <- additional_BAMM_parameters$updateLambdaInitScale
  } else {
  ...
  if ("updateLambdaShiftScale" %in% add_pars_names)
  {
    updateLambdaShiftScale <- additional_BAMM_parameters$updateLambdaShiftScale
  } else {
  ...
  if ("updateMuInitScale" %in% add_pars_names)
  {
    updateMuInitScale <- additional_BAMM_parameters$updateMuInitScale
  } else {
  ...
  if ("updateEventLocationScale" %in% add_pars_names)
  {
    updateEventLocationScale <- additional_BAMM_parameters$updateEventLocationScale
  } else {
  ...
  if ("updateEventRateScale" %in% add_pars_names)
  {
    updateEventRateScale <- additional_BAMM_parameters$updateEventRateScale
  } else {
  ...
  if ("updateRateEventNumber" %in% add_pars_names)
  {
    updateRateEventNumber <- additional_BAMM_parameters$updateRateEventNumber
  } else {
  ...
  if ("updateRateEventPosition" %in% add_pars_names)
  {
    updateRateEventPosition <- additional_BAMM_parameters$updateRateEventPosition
  } else {
  ...
  if ("updateRateEventRate" %in% add_pars_names)
  {
    updateRateEventRate <- additional_BAMM_parameters$updateRateEventRate
  } else {
  ...
  # Set the relative frequency of MCMC moves that change the initial speciation rates (lambda0) associated with a regime
    # lambda0 in lambda(t) = lamba0 x exp(alpha*t)
  if ("updateRateLambda0" %in% add_pars_names)
  {
    updateRateLambda0 <- additional_BAMM_parameters$updateRateLambda0
  } else {
  ...
  # Set the relative frequency of MCMC moves that change the exponential shift parameter (alpha) of the speciation rate associated with a regime
    # alpha in lambda(t) = lamba0 x exp(alpha*t)
  if ("updateRateLambdaShift" %in% add_pars_names)
  {
    updateRateLambdaShift <- additional_BAMM_parameters$updateRateLambdaShift
  } else {
  ...
  # Set the relative frequency of MCMC moves that change the (initial) extinction rate associated with a regime
    # mu0 in mu(t) = mu0 x exp(alpha*t)
    # As the extinction rates are actually assumed to follow constant rates, alpha is set to 0, thus mu(t) = mu0 and these are constant extinction rates
  if ("updateRateMu0" %in% add_pars_names)
  {
    updateRateMu0 <- additional_BAMM_parameters$updateRateMu0
  } else {
  ...
  if ("updateRateLambdaTimeMode" %in% add_pars_names)
  {
    updateRateLambdaTimeMode <- additional_BAMM_parameters$updateRateLambdaTimeMode
  } else {
  ...
  if ("localGlobalMoveRatio" %in% add_pars_names)
  {
    localGlobalMoveRatio <- additional_BAMM_parameters$localGlobalMoveRatio
  } else {
  ...
  # Set the initial speciation rate (lambda0) for the first regime starting at the root of the tree (regime 0)
    # lambda0 in lambda(t) = lamba0 x exp(alpha*t)
  if ("lambdaInit0" %in% add_pars_names)
  {
    lambdaInit0 <- additional_BAMM_parameters$lambdaInit0
  } else {
  ...
  # Set the initial shift parameter (alpha) for the root process (regime 0)
    # alpha in lambda(t) = lamba0 x exp(alpha*t)
  if ("lambdaShift0" %in% add_pars_names)
  {
    lambdaShift0 <- additional_BAMM_parameters$lambdaShift0
  } else {
  ...
  # Set the intial extinction rate (mu0) for the first regime starting at the root of the tree (regime 0)
    # mu0 in mu(t) = mu0 x exp(alpha*t)
    # As the extinction rates are actually assumed to follow constant rates, alpha is set to 0, thus mu(t) = mu0 and these are constant extinction rates
  if ("muInit0" %in% add_pars_names)
  {
    muInit0 <- additional_BAMM_parameters$muInit0
  } else {
  ...
  if ("initialNumberEvents" %in% add_pars_names)
  {
    initialNumberEvents <- additional_BAMM_parameters$initialNumberEvents
  } else {
  ...
  if ("numberOfChains" %in% add_pars_names)
  {
    numberOfChains <- additional_BAMM_parameters$numberOfChains
  } else {
  ...
  if ("deltaT" %in% add_pars_names)
  {
    deltaT <- additional_BAMM_parameters$deltaT
  } else {
    deltaT <- 0.01
  }
  ...
  if ("swapPeriod" %in% add_pars_names)
  {
    swapPeriod <- additional_BAMM_parameters$swapPeriod
  } else {
  ...
  if ("chainSwapFileName" %in% add_pars_names)
  {
    chainSwapFileName <- file.path(additional_BAMM_parameters$chainSwapFileName)
  } else {
  ...
  if ("minCladeSizeForShift" %in% add_pars_names)
  {
    minCladeSizeForShift <- additional_BAMM_parameters$minCladeSizeForShift
  } else {
  ...
  if ("segLength" %in% add_pars_names)
  {
    segLength <- additional_BAMM_parameters$segLength
  } else {
  ...
  if (is.null(prefix_for_files))
  {
    config_file_path <- file.path(paste0(BAMM_output_folder_path, "config_file.txt"))
  } else {
    config_file_path <- file.path(paste0(BAMM_output_folder_path, prefix_for_files,"_config_file.txt"))
  }
  ...
  ## run BAMM
  BAMM_path <- file.path(paste0(BAMM_install_folder_path, "bamm"))
  ...
  ## Clean outputs = move files to the dedicated folder
  # Detect output files
  if (is.null(prefix_for_files))
  {
    file.rename(from = runInfoFilename, to = file.path(paste0(BAMM_output_folder_path, runInfoFilename)))
    file.rename(from = mcmcOutfile, to = file.path(paste0(BAMM_output_folder_path, mcmcOutfile)))
    file.rename(from = eventDataOutfile, to = file.path(paste0(BAMM_output_folder_path, eventDataOutfile)))
    file.rename(from = chainSwapFileName, to = file.path(paste0(BAMM_output_folder_path, chainSwapFileName)))
    if (outputAcceptanceInfo == 1)
    {
      file.rename(from = acceptanceInfoFileName, to = file.path(paste0(BAMM_output_folder_path, acceptanceInfoFileName)))
    }
  } else {
    file.rename(from = file.path(paste0(prefix_for_files, "_", runInfoFilename)), to = file.path(paste0(BAMM_output_folder_path, prefix_for_files, "_", runInfoFilename)))
    file.rename(from = file.path(paste0(prefix_for_files, "_", mcmcOutfile)), to = file.path(paste0(BAMM_output_folder_path, prefix_for_files, "_", mcmcOutfile)))
    file.rename(from = file.path(paste0(prefix_for_files, "_", eventDataOutfile)), to = file.path(paste0(BAMM_output_folder_path, prefix_for_files, "_", eventDataOutfile)))
    file.rename(from = file.path(paste0(prefix_for_files, "_", chainSwapFileName)), to = file.path(paste0(BAMM_output_folder_path, prefix_for_files, "_", chainSwapFileName)))
    if (outputAcceptanceInfo == 1)
    {
      file.rename(from = acceptanceInfoFileName, to = file.path(paste0(BAMM_output_folder_path, prefix_for_files, "_", acceptanceInfoFileName)))
    }
  }
  ...
  if (!skip_evaluations)
  {
    ...
    if (is.null(prefix_for_files))
    {
      MCMC_log <- utils::read.csv(file.path(paste0(BAMM_output_folder_path, mcmcOutfile)), header = T)
    } else {
      MCMC_log <- utils::read.csv(file.path(paste0(BAMM_output_folder_path, prefix_for_files, "_", mcmcOutfile)), header = T)
    }
    ...
    if (save_evaluations)
    {
      if (is.null(prefix_for_files))
      {
        MCMC_logLik_path <- file.path(paste0(BAMM_output_folder_path, "MCMC_trace_logLik.pdf"))
      } else {
        MCMC_logLik_path <- file.path(paste0(BAMM_output_folder_path, prefix_for_files, "_MCMC_trace_logLik.pdf"))
      }
    ...
    if (save_evaluations)
    {
      if (is.null(prefix_for_files))
      {
        utils::write.csv(x = ESS_df, file = file.path(paste0(BAMM_output_folder_path, "ESS_df.csv")), row.names = FALSE)
      } else {
        utils::write.csv(x = ESS_df, file = file.path(paste0(BAMM_output_folder_path, prefix_for_files, "_ESS_df.csv")), row.names = FALSE)
      }
    }
    ...
    if (save_evaluations)
    {
      if (is.null(prefix_for_files))
      {
        PP_lambda_path <- file.path(paste0(BAMM_output_folder_path, "PP_lambda_plot.pdf"))
      } else {
        PP_lambda_path <- file.path(paste0(BAMM_output_folder_path, prefix_for_files, "_PP_lambda_plot.pdf"))
      }
  ...
  if (is.null(prefix_for_files))
  {
    eventData_path <- file.path(paste0(BAMM_output_folder_path, eventDataOutfile))
  } else {
    eventData_path <- file.path(paste0(BAMM_output_folder_path, prefix_for_files, "_", eventDataOutfile))
  }
  ...
  if(!keep_BAMM_outputs)
  {
    ...
    if (is.null(prefix_for_files))
    {
      file.remove(file.path(paste0(BAMM_output_folder_path, runInfoFilename)))
      file.remove(file.path(paste0(BAMM_output_folder_path, mcmcOutfile)))
      file.remove(file.path(paste0(BAMM_output_folder_path, eventDataOutfile)))
      file.remove(file.path(paste0(BAMM_output_folder_path, chainSwapFileName)))

      if (outputAcceptanceInfo == 1)
      {
        file.remove(file.path(paste0(BAMM_output_folder_path, acceptanceInfoFileName)))
      }
    } else {
      file.remove(file.path(paste0(BAMM_output_folder_path, prefix_for_files, "_", runInfoFilename)))
      file.remove(file.path(paste0(BAMM_output_folder_path, prefix_for_files, "_", mcmcOutfile)))
      file.remove(file.path(paste0(BAMM_output_folder_path, prefix_for_files, "_", eventDataOutfile)))
      file.remove(file.path(paste0(BAMM_output_folder_path, prefix_for_files, "_", chainSwapFileName)))
      if (outputAcceptanceInfo == 1)
      {
        file.remove(file.path(paste0(BAMM_output_folder_path, prefix_for_files, "_", acceptanceInfoFileName)))
      }
    }
  }
...
## Ignore BAMM folders during compilation
