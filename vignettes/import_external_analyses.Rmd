...
# Create fake categorical data for the whale example
whale_tip_cat_data <- c(rep(x = "small", times = 30),
                        rep(x = "medium", times = 20),
                        rep(x = "large", times = 37))
...
ER_fit <- geiger::fitDiscrete(phy = whale.tree,
...
whale_BAMM_object <- subset_BAMM_object(
...
length(whale_BAMM_object$eventData)
...
## Here is an example using the newly created 'densityMaps' object summarizing the ancestral characters of whales
# combined with our newly subsetted 'BAMM_object' build from the output from an external BAMM run.
