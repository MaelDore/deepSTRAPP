...
knitr::opts_chunk$set(
  eval = FALSE, # Chunks of codes will not be evaluated
  collapse = TRUE,
  comment = "#>",
...
* Step 3: Extract traits values, diversification rates, and regimes at a given time in the past
...
This is a simple example that shows how deepSTRAPP can be used to test for differences in diversification rates between traits along evolutionary times.
...
**Need to find a simple example with enough regime shifts to find significant results, and ideally, non-significant results in the present, but a significant time-frame in the past**
...
```{r prepare_diversification_data}
...
```{r plot_results, eval = TRUE, echo = TRUE}
# ------ Step 4: Plot results ------ #
...
```
```{r plot_histogram_STRAPP_tests_eval, eval = TRUE, echo = FALSE}
# Plot the histogram of test stats for time-step n°3 = 20 My
plot_histogram_STRAPP_test_for_focal_time(
   deepSTRAPP_outputs = Ponerinae_deepSTRAPP_cont_0_40,
   focal_time = 20)
...
```
```{r plot_rates_through_time_eval, eval = TRUE, echo = FALSE}
plot_rates_through_time(deepSTRAPP_outputs = Ponerinae_deepSTRAPP_cont_0_40)
```

```{r plot_updated_contMap, eval = TRUE, echo = TRUE}
### 4/ Plot updated contMap mapping trait evolution for a given 'focal_time' ####
...
```
```{r plot_BAMM_rates_eval, eval = TRUE, echo = FALSE}
# Plot diversification rates on initial phylogeny (t = 0)
BAMM_map_0My <- Ponerinae_deepSTRAPP_cont_0_40$updated_BAMM_objects_over_time[[1]]
plot_BAMM_rates(BAMM_map_0My, labels = FALSE, legend = TRUE, par.reset = FALSE)
abline(v = max(phytools::nodeHeights(Ponerinae_tree)[,2]) - 20, col = "red", lty = 2) # Show where the phylogeny will be cut
...
* Plot traits: contMap, DensityMaps per states/ranges, DensityMap with alpha
