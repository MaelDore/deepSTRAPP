...
- contMaps:
  ...
  List of objects of class `"contMap"`, typically generated with
...
The function returns an updated list of objects as cut contMaps of class
`"contMap"`.
...
- `$lims` A numeric vector of length 2 storing the initial min and max
  values of the trait (trait values are scaled between 0 and 1000 in
  `$tree$maps`)
...
The continuous trait mappings are updated accordingly by removing
